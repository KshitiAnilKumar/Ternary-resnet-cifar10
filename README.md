...
The official test set is kept untouched during training and is used only for final evaluation.
...
The large experiment output ZIP files are provided through GitHub Releases rather than stored directly in the repository.
...
The trained teacher is used to provide soft predictions for knowledge distillation.

### 02 — FP32 ResNet-18 Baseline

Trains a standard full-precision ResNet-18 without knowledge distillation or quantization.

This provides the baseline for comparison with the ternary student.

### 03 — Ternary KD/QAT Student

Trains the ResNet-18 student using:
...
The internal convolutional weights are quantized to:

```
{-α, 0, +α}
```

The main knowledge-distillation configuration uses:
...
| Model | Test Accuracy | Parameters | Checkpoint Size |
|---|---|---|---|
...
The final ternary student achieves 95.32% test accuracy while reducing the FP32 ResNet-18 checkpoint size by approximately 15.135×.

The ternary convolutional/fully connected weights have approximately 45.5% sparsity.
...
Because the archives are large, they are provided through GitHub Releases.
...
Use the notebooks in the order listed above.
...
- The official CIFAR-10 test set is not used for training or model selection.
- The teacher is frozen during knowledge-distillation training.
- The ternary student uses ternary weights during the forward pass.
- Latent FP32 weights are maintained for optimization.
- The reported compression refers to model storage and does not imply an equivalent inference-speed improvement.
